  ...
  )
{
  ...
  for (Node = FdtNextNode (Fdt, 0, &Depth); Node >= 0; Node = FdtNextNode (Fdt, Node, &Depth)) {
 ...
 #if 1
      if ((FreeMemoryBottom  - NumberOfBytes + MinimalNeededSize) == StartAddress ) {
        // StartAddress  +=  MinimalNeededSize;
        // StartAddress = MemoryBottom - NumberOfBytes + MinimalNeededSize;
    ...
    // Optional
      ...
      if (PciRootBridgeInfo == NULL) {
        PciRootBridgeInfo = BuildGuidHob (&gUniversalPayloadPciRootBridgeInfoGuid, sizeof (UNIVERSAL_PAYLOAD_PCI_ROOT_BRIDGES) + RootBridgeCount *sizeof (UNIVERSAL_PAYLOAD_PCI_ROOT_BRIDGE));
